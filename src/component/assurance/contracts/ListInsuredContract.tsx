...
import { InsuredContractTable } from '../../tables/Table.tsx'
import {institutionsDataSelector} from "../../../features/institutionSlice.ts"
import { useSelector } from 'react-redux'
import { useForm } from 'react-hook-form'
import { listAssuranceContratThunk,
...
const ListInsuredContract = () => {
    ...
    const [contratState, setContratState] = useState<StatInsuredCompany>()
    ...
    async function handleGetContratState(data) {
        ...
        try {
            ...
            //console.log("tttttttttt",response?.payload)
                
                
           if(response){
            setLoading(false)
            //setContratState(response?.payload)
                //console.log("aaaaaaaaaaaa", contratState);
           }
        } catch (error) {
            console.log(error)
            setLoading(false)
            ...
            <div className="list-patients">
                <div className="feature-title">
                    ...
                    <div className="btn-group">
                        <Link to={`/create-contrat/${params.code}`} className='btn-link-green'
                ...
                {contrat?.length > 0 ?
                    <div className="patients">
                        <InsuredContractTable data={contrat}
                        />
                    </div>
                    :
...
export default ListInsuredContract